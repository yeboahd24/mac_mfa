...
def get_totp_device(user):
    ...
    if totp_device:
        ...
        return create_totp_device(user)


def get_totp_secret(user):
    totp_device = get_user_totp_device(user)
    if totp_device:
        return totp_device.secret
    else:
        return None


def get_totp_qr_code(user):
    totp_device = get_user_totp_device(user)
    if totp_device:
...
class MFACreateView(View):
    def get(self, request):
        try:
            ...
            totp_device = get_totp_device(user)
...
class MFAVerifyView(View):
    def post(self, request, token, format=None):
        try:
            ...
            user = TokenService.get_user_from_token(user_token)
            totp_device = get_totp_device(user)
            print(token)
            print(totp_device.verify_is_allowed())
            ...
            if not totp_device == None and totp_device.verify_token(token):
                if not totp_device.confirmed:
                    ...
                    totp_device.save()
                    return JsonResponse(
                        {"message": "TOTP device verified successfully"}, status=200
                    )
                else:
                    return JsonResponse(
                        {"error": "TOTP device already verified"}, status=400
                    )
            else:
